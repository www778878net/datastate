...
impl DataStateMysql {
    ...
    /// - 权限检查：验证caller是否有权限调用此方法
    /// - 审计日志：通过log_action_with_count记录操作摘要
    pub async fn get(&self, where_clause: &str, params: Vec<Value>, caller: &str, summary: &str) -> Result<Vec<HashMap<String, Value>>, String> {
    ...
    /// - 权限检查：验证caller是否有权限调用此方法
    /// - 审计日志：通过log_action_with_count记录操作摘要
    pub async fn do_get(&self, sql: &str, params: Vec<Value>, caller: &str, summary: &str) -> Result<Vec<HashMap<String, Value>>, String> {
    ...
    /// - 审计日志：通过log_action_with_count记录操作摘要
    /// - 返回影响的行数
    pub async fn do_m(&self, sql: &str, params: Vec<Value>, caller: &str, summary: &str) -> Result<usize, String> {
