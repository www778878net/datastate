...
fn test_01_generate_synclog() {
    ...
    for (i, item) in items.iter().enumerate() {
        println!("  [{}] action={}, idrow={}", i, item.action, item.idrow);
    }
...
fn test_02_sync_to_mysql_synclog() {
    ...
    if items.is_empty() {
        println!("没有待同步记录，跳过测试");
        return;
    }

    println!("待同步记录: {} 条", items.len());

    // 调用 API 上传
    println!("\n【调用 axum78 API】");
    match upload_synclog_to_api(&items, DEFAULT_SID) {
        Ok((success_ids, failed)) => {
            println!("成功: {} 条", success_ids.len());
            println!("失败: {} 条", failed.len());

            for id in &success_ids {
                println!("  ✓ 成功: {}", id);
            }
            for err in &failed {
                println!("  ✗ 失败: {} - {} ({})", err.idrow, err.error, err.id);
            }
        ...
        Err(e) => {
            println!("API 调用失败: {}", e);
            println!("请确保 axum78 服务已启动: {}", API_BASE_URL);
        }
    }
...
fn test_03_verify_mysql_testtb() {
    ...
    println!("本地 testtb 记录: {} 条", local_records.len());

    // 查询 MySQL 数据（通过 API）
    println!("\n【查询 MySQL testtb】");
    match query_mysql_testtb_via_api(DEFAULT_SID, "test_01") {
        Ok(mysql_records) => {
            println!("MySQL testtb 记录: {} 条", mysql_records.len());
            ...
            for local in &local_records {
                ...
                if let Some(mysql) = mysql_match {
                    ...
                    println!("  {} id={}: local={}, mysql={}", match_status, local_id, local_data, mysql_data);
                } else {
                    println!("  ✗ id={}: MySQL 中不存在", local_id);
                }
            }
        }
        Err(e) => {
            println!("查询 MySQL 失败: {}", e);
            println!("请确保 axum78 服务已启动: {}", API_BASE_URL);
        }
    }
...
fn test_04_download_from_mysql() {
    ...
    println!("【从 axum78 下载 synclog】");
    match download_synclog_from_api(DEFAULT_SID, 100) {
        Ok(items) => {
            println!("下载到 {} 条记录", items.len());

            for item in &items {
                println!("  - action={}, tbname={}, idrow={}", item.action, item.tbname, item.idrow);
...
fn test_05_conflict_resolution() {
    ...
    println!("本地修改: data=local_modified, uptime={}", local_time);

    // 模拟 MySQL 端修改（需要通过 API 或直接操作数据库）
    std::thread::sleep(std::time::Duration::from_millis(100));
    let server_time = Local::now().format("%Y-%m-%d %H:%M:%S").to_string();
    println!("模拟服务器修改: data=server_modified, uptime={}", server_time);
    println!("（实际需要在 MySQL 端执行 UPDATE）");

    // 冲突解决逻辑：比较 uptime，较新的覆盖较旧的
    println!("\n【冲突解决策略】");
    println!("比较 uptime:");
    println!("  本地: {}", local_time);
    println!("  服务器: {}", server_time);
    
    if server_time > local_time {
        println!("=> 服务器数据更新，本地接受服务器数据");
    } else {
        println!("=> 本地数据更新，保持本地数据");
    }
...
fn test_06_account_isolation() {
    ...
    println!("默认用户创建: id={}, cid={}", default_id, DEFAULT_CID);
    ...
    println!("guest3 用户创建: id={}, cid={}", guest3_id, GUEST3_CID);
    ...
    match upload_synclog_to_api(&items, DEFAULT_SID) {
        Ok((success_ids, failed)) => {
            ...
            if guest3_failed {
                println!("✓ guest3 的记录被正确拒绝（权限隔离生效）");
            }

            for err in &failed {
                println!("  ✗ 失败: {} - {}", err.idrow, err.error);
            }
...
fn test_full_workflow() {
    println!("\n========== 完整同步流程测试 ==========\n");
    println!("架构: 本地 SQLite -> axum78 API -> MySQL");
    println!("API: {}", SYNCLOG_API_URL);
    println!();
    ...
    match upload_synclog_to_api(&items, DEFAULT_SID) {
        Ok((success_ids, failed)) => {
            ...
            println!("本地记录: {} 条", local_records.len());

            match query_mysql_testtb_via_api(DEFAULT_SID, "test_full") {
                Ok(mysql_records) => {
                    println!("MySQL 记录: {} 条", mysql_records.len());
